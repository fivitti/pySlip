...
TileURLPath = '/com.modestmaps.bluemarble/%d-r%d-c%d.jpg'   # zoom, row, column
TileLevels = range(10)
...
# this can be overridden in the BMTiles() constructor
DefaultTilesDir = 'bm_tiles'
...
######
# Override the pyCacheBack object to handle BlueMarble tile retrieval
######

class BMCache(pycacheback.pyCacheBack):
...
class TileWorker(threading.Thread):
    ...
    def run(self):
        while True:
            ...
            try:
                tile_url = self.server + self.tilepath % (level, y, x)
...
################################################################################
# Class for BlueMarble tiles.   Builds on tiles.Tiles.
################################################################################

class BMTiles(tiles.Tiles):
    """An object to source BlueMarble tiles for pySlip."""
    ...
    def __init__(self, tiles_dir=None, tile_levels=None, callback=None,
        ...
        # tiles extent for BlueMarble tile data (left, right, top, bottom)
        ...
        self.cache = BMCache(tiles_dir=self.tiles_dir, max_lru=DefaultMaxLRU)

        # set the list of queued unsatisfied requests to 'empty'
        self.queued_requests = {}

        # BlueMarble tiles always (256, 256)
...
if __name__ == '__main__':
    ...
    DemoName = 'BlueMarble Tiles Test'
    DemoVersion = '0.1'
    ...
    class TestBMTiles(unittest.TestCase):

        def test_Tile2Geo(self):
            """Exercise tiles.Tile2Geo() at various known places."""

            tiles = BMTiles(tiles_dir=DefaultTilesDir)
            tiles.UseLevel(2)
