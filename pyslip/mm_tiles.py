...
"""
A tile source that serves ModestMaps tiles from the internet.
...
######
# Override the pyCacheBack object to handle ModestMaps tile retrieval
######

class MMCache(pycacheback.pyCacheBack):
...
################################################################################
# Class for MM tiles.   Builds on tiles.Tiles.
################################################################################

class MMTiles(tiles.Tiles):
    """An object to source ModestMaps tiles for pySlip."""
    ...
    def __init__(self, tiles_dir=None, tile_levels=None, callback=None,
        ...
        # tiles extent for MM tile data (left, right, top, bottom)
        ...
        self.cache = MMCache(tiles_dir=self.tiles_dir, max_lru=DefaultMaxLRU)
...
if __name__ == '__main__':
    ...
    class TestMMTiles(unittest.TestCase):

        def test_Tile2Geo(self):
            """Exercise tiles.Tile2Geo() at various known places."""

            tiles = MMTiles(tiles_dir=DefaultTilesDir)
            tiles.UseLevel(2)
