...
TileURLPath = '/tiles/1.0.0/map/%d/%d/%d.jpg'      # zoom, column, row
TileLevels = range(17)
...
# this can be overridden in the MQTiles() constructor
DefaultTilesDir = 'mq_tiles'
...
######
# Override the pyCacheBack object to handle MapQuest tile retrieval
######

class MQCache(pycacheback.pyCacheBack):
...
################################################################################
# Class for MQ tiles.   Builds on tiles.Tiles.
################################################################################

class MQTiles(tiles.Tiles):
    """An object to source MapQuest tiles for pySlip."""
    ...
    def __init__(self, tiles_dir=None, tile_levels=None, callback=None,
        ...
        # tiles extent for MQ tile data (left, right, top, bottom)
        ...
        self.cache = MQCache(tiles_dir=self.tiles_dir, max_lru=DefaultMaxLRU)

        # set the list of queued unsatisfied requests to 'empty'
        self.queued_requests = {}

        # MapQuest tiles always (256, 256)
...
if __name__ == '__main__':
    ...
    class TestMQTiles(unittest.TestCase):

        def test_Tile2Geo(self):
            """Exercise tiles.Tile2Geo() at various known places."""

            tiles = MQTiles(tiles_dir=DefaultTilesDir)
            tiles.UseLevel(2)
