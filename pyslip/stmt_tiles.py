...
TilesetShortName = 'STMToner Tiles'
TilesetVersion = '1.0'

# the pool of tile servers used and tile path on server
# to each tile, %params are (level, x, y)
TileServers = ['http://tile.stamen.com/toner',
              ]
TileURLPath = '/%d/%d/%d.png'   # zoom, column, row
TileLevels = range(17)
...
# this can be overridden in the STMTTiles() constructor
...
######
# Override the pyCacheBack object to handle STMT tile retrieval
######

class STMTCache(pycacheback.pyCacheBack):
    ...
    def _put_to_back(self, key, value):
        ...
        try:
            ...
            pass
        value.SaveFile(tile_path, wx.BITMAP_TYPE_PNG)
    ...
    def run(self):
        while True:
            ...
            try:
                ...
                if f.info().getheader('Content-Type') == 'image/png':
                    image = wx.ImageFromStream(f, wx.BITMAP_TYPE_PNG)
...
################################################################################
# Class for STMT tiles.   Builds on tiles.Tiles.
################################################################################

class STMTTiles(tiles.Tiles):
    """An object to source STMT tiles for pySlip."""
    ...
    def __init__(self, tiles_dir=None, tile_levels=None, callback=None,
        ...
        # tiles extent for STMT tile data (left, right, top, bottom)
        ...
        self.cache = STMTCache(tiles_dir=self.tiles_dir, max_lru=DefaultMaxLRU)

        # set the list of queued unsatisfied requests to 'empty'
        self.queued_requests = {}

        # STMT tiles always (256, 256)
...
if __name__ == '__main__':
    ...
    class TestSTMTiles(unittest.TestCase):

        def test_Tile2Geo(self):
            """Exercise tiles.Tile2Geo() at various known places."""

            tiles = STMTiles(tiles_dir=DefaultTilesDir)
            tiles.UseLevel(2)
