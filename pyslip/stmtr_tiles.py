...
TileServers = ['http://a.tile2.opencyclemap.org/transport',
              ]
...
# this can be overridden in the STMTRTiles() constructor
...
######
# Override the pyCacheBack object to handle STMTR tile retrieval
######

class STMTRCache(pycacheback.pyCacheBack):
...
################################################################################
# Class for STMTR tiles.   Builds on tiles.Tiles.
################################################################################

class STMTRTiles(tiles.Tiles):
    """An object to source STMTR tiles for pySlip."""
    ...
    def __init__(self, tiles_dir=None, tile_levels=None, callback=None,
        ...
        # tiles extent for STMTR tile data (left, right, top, bottom)
        ...
        self.cache = STMTRCache(tiles_dir=self.tiles_dir, max_lru=DefaultMaxLRU)

        # set the list of queued unsatisfied requests to 'empty'
        self.queued_requests = {}

        # STMTR tiles always (256, 256)
...
if __name__ == '__main__':
    ...
    class TestSTMTRTiles(unittest.TestCase):

        def test_Tile2Geo(self):
            """Exercise tiles.Tile2Geo() at various known places."""

            tiles = STMTRTiles(tiles_dir=DefaultTilesDir)
            tiles.UseLevel(2)
