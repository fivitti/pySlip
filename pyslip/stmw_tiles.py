...
# this can be overridden in the STMWTiles() constructor
...
######
# Override the pyCacheBack object to handle STMW tile retrieval
######

class STMWCache(pycacheback.pyCacheBack):
    ...
    def run(self):
        while True:
            ...
            try:
                ...
                log("run: f.info().getheader('Content-Type')=%s" % str(f.info().getheader('Content-Type')))
...
################################################################################
# Class for STMW tiles.   Builds on tiles.Tiles.
################################################################################

class STMWTiles(tiles.Tiles):
    """An object to source STMW tiles for pySlip."""
    ...
    def __init__(self, tiles_dir=None, tile_levels=None, callback=None,
        ...
        # tiles extent for STMW tile data (left, right, top, bottom)
        ...
        self.cache = STMWCache(tiles_dir=self.tiles_dir, max_lru=DefaultMaxLRU)

        # set the list of queued unsatisfied requests to 'empty'
        self.queued_requests = {}

        # STMW tiles always (256, 256)
...
if __name__ == '__main__':
    ...
    class TestSTMWTiles(unittest.TestCase):

        def test_Tile2Geo(self):
            """Exercise tiles.Tile2Geo() at various known places."""

            tiles = STMWTiles(tiles_dir=DefaultTilesDir)
            tiles.UseLevel(2)
