...
The idea is to have a set of radiobuttons selecting various
geo positions on the OSM tile map.  When selected, the view
would be moved with GotoPosition() and a marker would be drawn
at that position.  At the same time, a map-relative marker
would be drawn at the centre of the view.  The differnce between
the two markers shows errors in the geo2view() functions.
"""
...
# demo name/version
DemoName = "pySlip %s - 'Net Demonstration" % pyslip.__version__
DemoVersion = '2.0'
...
#MRPointShowLevels = range(17)
#MRImageShowLevels = range(17)
#MRTextShowLevels = range(17)
#MRPolyShowLevels = range(17)
MRPointShowLevels = None
MRImageShowLevels = None
MRTextShowLevels = None
MRPolyShowLevels = None
...
################################################################################
# Make a Tile object for MapQuest tiles
################################################################################
...
#    TileLevels = range(13)         # [0, ..., 12] for the satellite tiles
    ...
    # path to tile in on-disk cache, %params are tile (x, y) coords
    TilePath = os.path.join('%d', '%d.jpg')
    ...
    def GetInfo(self, level):
        ...
        return (2**level, 2**level,
                (2**level * self.TileSize)/(self.extent[1] - self.extent[0]),
                (2**level * self.TileSize)/(self.extent[3] - self.extent[2]))
    ...
    def ConvertGeo2TileCoords(self, lat_deg, lon_deg, zoom,
        ...
        with the returned results being tile *fractional* values.  The expectation is that
        the integer part will be the tile coordinate and the fractional part is the fraction
        of the tile.

        Note: This code is incorrect in the integer tile coord part for latitude for
              absolute latitudes greater than about 85 degrees.  Further testing is
              needed to determine how accurate the fractional part is.
        """

        lat_rad = math.radians(lat_deg)
        n = 2.0 ** zoom
        xtile = (lon_deg - LeftLon) / MapWidth * n
        try:
    ...
    def ConvertTileCoords2Geo(self, xtile, ytile, zoom):
        ...
        This code is taken from:
            http://wiki.openstreetmap.org/wiki/Slippy_map_tilenames#Lon..2Flat._to_tile_numbers_2
        with the input args being tile *fractional* values.  The expectation is that
        the integer part will be the tile coordinate and the fractional part is the fraction
        of the tile.
        ...
        n = 2.0 ** self.level
        lon_deg = (MapWidth * xtile)/n + LeftLon
...
#        return (lat_deg, lon_deg)
    ...
    def ConvertGeo2View(self, lonlat, view_offset_x, view_offset_y, view_llon,
        ...
        Return screen pixels coordinates of the point (lon,lat).
        ...
        n = 2.0 ** self.level       # number of tiles in the level width or height
        x = int((lon+180.0)*self.TileSize/360.0*n)
        y = int((1.0 - math.log(math.tan(lat_rad) + (1/math.cos(lat_rad))) / math.pi) * float(self.TileSize)/2.0*n)

        return (x-view_offset_x, y-view_offset_y)
    ...
    def ConvertView2Geo(self, xy, view_offset_x, view_offset_y, map_llon,
        ...
        # convert view X & Y to tile coordinates
        xtile = (view_offset_x + x)/self.tile_size_x
        ytile = (view_offset_y + y)/self.tile_size_y

        return self.ConvertTileCoords2Geo(xtile, ytile, self.level)

    def GetTile(self, x, y):
        """Get bitmap for tile at tile coords (x, y), current level.

        x  X coord of tile required (tile coordinates)
        y  Y coord of tile required (tile coordinates)

        Look for the tile in the local cache, return if found.  If *not* local,
        start getting from servers and return 'pending' tile bitmap.
        """

        tile = self.cache.GetTile(self.level, x, y)
        if tile is None:
    ...
    def GetInternetTile(self, level, x, y):
        ...
        do this as we can't peek into a Queue to see what's there.
        """

        tile_id = (level, x, y)
        if tile_id not in self.queued_requests:
            # add tile request to the server request queue
            self.request_queue.put(tile_id)
            self.queued_requests[tile_id] = True
    ...
    def _tile_available(self, level, x, y, image):
        ...
        self.cache.CacheTile(image, bitmap, level, x, y)
...
class TileWorker(threading.Thread):
    """Thread class that gets request from a queue, loads the tile,
...
#####
    ...
    def make_gui_view(self, parent):
        ...
        tile_object = MyTiles(self.tile_directory, MyTiles.TileLevels,
    ...
    def make_gui_point(self, parent):
        ...
        # create widgets
        point_obj = LayerControl(parent, 'Points, map relative', selectable=True)
    ...
    def make_gui_image(self, parent):
        ...
        # create widgets
        image_obj = LayerControl(parent, 'Images, map relative', selectable=True)
    ...
    def make_gui_poly(self, parent):
        ...
        # create widgets
        poly_obj = LayerControl(parent, 'Polygon, map relative', selectable=True)
...
##### view-relative polygon layer
    ...
    def init(self):
        ...
        capital = {'placement': 'se', 'fontsize': 14, 'color': 'red',
                   'textcolour': 'red'}
        TextData = [(151.216, -33.855, 'Sydney', text_placement),
                    (144.963, -37.814, 'Melbourne', {'placement': 'ce'}),
                    (153.024, -27.469, 'Brisbane', text_placement),
                    (115.860, -31.953, 'Perth', transparent_placement),
                    (138.600, -34.929, 'Adelaide', text_placement),
                    (130.841, -12.460, 'Darwin', text_placement),
                    (147.328, -42.883, 'Hobart', text_placement),
                    (174.765, -36.854, 'Auckland', text_placement),
                    (174.781, -41.287, 'Wellington', capital),
                    (172.637, -43.531, 'Christchurch', text_placement),
                    (168.661, -45.032, 'Queenstown', text_placement),
                    (147.150, -09.479, 'Port Moresby', capital),
                    (106.828, -06.181, 'Jakarta', capital),
                    (110.374, -07.785, 'Yogyakarta', text_placement),
                    (120.980, +14.591, 'Manila', capital),
                    (-88.244, +40.117, 'Champaign', text_placement),
                    (160.000, -30.000, 'Agnes Napier - 1855',
                        {'placement': 'cw', 'offset_x': 20, 'color': 'green'}),
                    (145.000, -11.000, 'Venus - 1826',
                        {'placement': 'sw', 'color': 'green'}),
                    (156.000, -23.000, 'Wolverine - 1879',
                        {'placement': 'ce', 'color': 'green'}),
                    (150.000, -15.000, 'Thomas Day - 1884',
                        {'color': 'green'}),
                    (165.000, -19.000, 'Sybil - 1902',
                        {'placement': 'cw', 'color': 'green'}),
                    (158.550, -19.980, 'Prince of Denmark - 1863',
        ...
        if sys.platform != 'win32':
            TextData.extend([
                    (106.367, +10.360, 'Mỹ Tho', {'placement': 'ne'}),
                    (105.852, +21.029, 'Hà Nội', capital),
                    (106.701, +10.785, 'Thành phố Hồ Chí Minh', {'placement': 'sw'}),
                    (132.452, +34.392, '広島市 (Hiroshima City)', text_placement),
                    (114.167, +22.264, '香港 (Hong Kong)', {'placement': 'nw'}),
                    ( 96.150, +16.798, 'ရန်ကုန် (Rangoon)', capital),
                    (104.933, +11.563, ' ភ្នំពេញ (Phnom Penh)', 
                        {'placement': 'ce', 'fontsize': 12, 'color': 'red'}),
                    (100.496, +13.760, 'กรุงเทพมหานคร (Bangkok)', capital),
                    ( 77.582, +34.163, 'གླེ་(Leh)', text_placement),
                    ( 84.991, +24.697, 'बोधगया (Bodh Gaya)', text_placement)])
